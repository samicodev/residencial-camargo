...
require_once('../modelo/conexion.php');
    ...
    $pdf->Cell(10); // Espacio vacío en el medio
    $pdf->Cell(60, 7, utf8_decode('Huésped: ' . $resultado['nombcomp']), 0, 0, 'L'); 
    $pdf->Cell(70); // Espacio vacío en el medio
    ...
    $pdf->Cell(80, 10, utf8_decode('Cobro de Servicio de Hospedaje Durante la Estancia.'), 1, 0, 'C', 0); // Datos para la columna DESCRIPCIÓN
    ...
    $pdfContent = $pdf->Output('S');  // El parámetro 'S' indica que se genera el contenido en memoria (string)
    ...
    try {
        ...
        $mail->SMTPAuth   = true;                                   
        $mail->Username   = 'samuel.mamani.inc@gmail.com';  // Tu correo
        $mail->Password   = 'cmhv jppx vlor hzho';  // Tu contraseña
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;         
        $mail->Port       = 587;  // Puerto del servidor SMTP

        // Configuración del correo
        $mail->setFrom('samuel.mamani.inc@gmail.com', 'Residencial Camargo');  // Correo del remitente
        $mail->addAddress($email);  // Correo del destinatario

        // Adjuntar el PDF generado en memoria
        $mail->addStringAttachment($pdfContent, 'comprobantePago.pdf');  // Adjuntar el contenido en memoria como un archivo PDF

        // Contenido del correo
        $mail->isHTML(true);                                  
        $mail->Subject = 'Comprobante de Pago Residencial Camargo';
        $mail->Body    = 'Le envío adjunto el comprobante de pago correspondiente a su hospedaje en el Residencial Camargo. Conserve este comprobante para futuras referencias.';

        // Enviar el correo
        $mail->send();
        echo json_encode(['success' => true, 'message' => 'El correo ha sido enviado con éxito']);
    } catch (Exception $e) {
        echo json_encode(['success' => false, 'message' => "No se pudo enviar el comprobante por correo"]);
    }
