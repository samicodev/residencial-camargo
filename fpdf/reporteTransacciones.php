...
require_once('../modelo/conexion.php'); //llamamos a la conexion BD
   ...
   if (isset($_POST['idcajatransaccion']) && isset($_POST['BusqFechIni']) && isset($_POST['BusqFechFin'])) {
      $idcaja = $_POST['idcajatransaccion'];
      $fechainicio = $_POST['BusqFechIni'];
      $fechafin = $_POST['BusqFechFin'];
      ...
      if (!empty($fechainicio) && !empty($fechafin)) {
          ...
          $fechaFormateadaInicio = $fechaInicioObj->format('d/m/Y');
          $fechaFormateadaFin = $fechaFinObj->format('d/m/Y');

          $rangoFechasBus = "($fechaFormateadaInicio - $fechaFormateadaFin)";
      }
  ...
  $fechaInicioCaja = DateTime::createFromFormat('d/m/Y h:i A', $resultado[0]['fechaaperturacaja'])->format('d/m/Y');
  $fechaFinCaja = empty($resultado[0]['fechacierrecaja']) ? 'En curso' : DateTime::createFromFormat('d/m/Y h:i A', $resultado[0]['fechacierrecaja'])->format('d/m/Y');
    $rangoFechas='';
  if(!empty($rangoFechasBus)){
   $rangoFechas="$rangoFechasBus";
  }else{
     $rangoFechas = "($fechaInicioCaja - $fechaFinCaja)";
  }
...
$pdf->Cell(190, 7, utf8_decode("REPORTE - FLUJO DE CAJA $rangoFechas"), 1, 1, 'C', 1);
$pdf->Ln(0);
...
$pdf->Cell(20, 7, utf8_decode('Ingresos'), 1, 1, 'C', 1);
   ...
   if(!empty($resultado)){
      ...
      $pdf->SetFont('Times', 'B', 9); // Cambiamos la fuente a negrita
      $pdf->Cell(150, 7, utf8_decode('SALDO FINAL'), 1, 0, 'C', 1); // Columna de subtotales
      $pdf->Cell(40, 7, utf8_decode(number_format($totalIngresos - $totalEgresos, 2)), 1, 1, 'C', 0); // Total de ingresos + saldo inidcial - egresos
   }
